...
pub fn version_num() -> u32 {
	let v1 = (_API.SciterVersion)(1);
	let v2 = (_API.SciterVersion)(0);
	let (major, minor, revision, _build) = (v1 >> 16 & 0xFF, v1 & 0xFF, v2 >> 16 & 0xFF, v2 & 0xFF);
	let num = (major << 24) | (minor << 16) | (revision << 8);

	// version format changed in a version idk which.
	if num < 0x0400_0100 {
		let version = ((_API.SciterVersion)(0) as u32) << 24 | ((_API.SciterVersion)(1) as u32) << 16 | ((_API.SciterVersion)(2) as u32) << 8;
		return version;
	}
